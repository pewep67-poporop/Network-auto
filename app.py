...
def add_log(level: str, module: str, message: str):
    ...
    # Ghi file log không được làm crash toàn bộ app nếu không có quyền ghi
    try:
...
def get_real_error(e: Exception) -> str:
    """
    Nornir boc loi cua subtask (task.run() goi long ben trong 1 task khac)
    thanh NornirSubTaskError, khien str(e) chi tra ve chuoi chung chung
    'Subtask: netmiko_send_command (failed)' va che mat loi that su
    (vi du: NetmikoAuthenticationException, NetmikoTimeoutException,
    TypeError do truyen sai tham so...).

    Ham nay "boc vo" de lay ra loi goc, phuc vu debug chinh xac.
    """
    try:
        from nornir.core.exceptions import NornirSubTaskError
        if isinstance(e, NornirSubTaskError) and e.result is not None:
            inner = e.result[0].exception if len(e.result) else getattr(e.result, "exception", None)
...
def engine_backup():
    ...
    try:
        ...
        add_log("INFO", "BACKUP", f"Tìm thấy {len(hosts)} thiết bị: {hosts}")

        if not hosts:
            add_log("WARNING", "BACKUP", "Inventory rỗng, không có thiết bị nào để backup!")
            return

        def backup_task(task):
            try:
                # CHI truyen cac tham so ma send_command() thuc su ho tro.
                # 'delay_factor' (khong phai 'global_delay_factor') giup cho
                # thiet bi cham (IOL/dynamips trong PNetLab) co du thoi gian tra loi.
                # Cac tham so ket noi (timeout, disabled_algorithms cho SSH cipher cu...)
                # phai khai bao trong inventory (hosts.yaml/defaults.yaml), KHONG duoc
                # truyen o day vi se gay TypeError bi Nornir che thanh loi chung chung.
                result = task.run(
                ...
                add_log("SUCCESS", "BACKUP", f"OK {task.host.name} -> {filepath.name}")
                return "OK"
            except Exception as e:
                add_log("ERROR", "BACKUP", f"FAIL {task.host.name}: {get_real_error(e)}")
                return f"FAIL: {get_real_error(e)}"
        ...
        add_log("ERROR", "BACKUP", f"Lỗi hệ thống: {get_real_error(e)}")
    finally:
...
def engine_discovery():
    ...
    try:
        ...
        hosts = list(nr.inventory.hosts.keys())
        add_log("INFO", "DISCOVERY", f"Tìm thấy {len(hosts)} thiết bị: {hosts}")

        if not hosts:
            add_log("WARNING", "DISCOVERY", "Inventory rỗng, không có thiết bị nào để quét!")
            return
        ...
        def discovery_task(task):
            try:
                ...
                add_log("SUCCESS", "DISCOVERY",
                        f"OK {task.host.name} | Model: {model} | Uptime: {uptime}")
                return {"model": model, "uptime": uptime}
            except Exception as e:
                add_log("ERROR", "DISCOVERY", f"FAIL {task.host.name}: {get_real_error(e)}")
                return {"model": "Unknown", "error": get_real_error(e)}

        results = nr.run(task=discovery_task)
        success = sum(1 for r in results.values() if not r.failed)
        failed = sum(1 for r in results.values() if r.failed)
        add_log("INFO", "DISCOVERY", f"Quét xong {len(results)} thiết bị ({success} OK, {failed} lỗi)")

    except Exception as e:
        add_log("ERROR", "DISCOVERY", f"Lỗi hệ thống: {get_real_error(e)}")
    finally:
...
def engine_deploy(device_name: str, config_type: str, params: dict, dry_run: bool = True):
    ...
    try:
        ...
        if not dry_run:
            ...
            def push_task(task):
                try:
                    ...
                    add_log("ERROR", "DEPLOY", f"FAIL {task.host.name}: {get_real_error(e)}")
        ...
        add_log("ERROR", "DEPLOY", f"Lỗi: {get_real_error(e)}")
        return None
    finally:
...
class NetworkScanner:
    ...
    def scan_subnet(subnet: str, username: str = "admin", password: str = "") -> List[Dict]:
        ...
        with ThreadPoolExecutor(max_workers=50) as executor:
            list(executor.map(check_ip, network.hosts()))
...
def api_add_device(device: DeviceInput):
    ...
    if not device.password:
        add_log("WARNING", "INVENTORY",
                f"Thêm '{device.name}' với password RỖNG - kết nối SSH sẽ luôn thất bại!")
...
def api_delete_device(device_name: str):
    ...
    save_inventory(inventory)
    add_log("INFO", "INVENTORY", f"Xóa thiết bị: {device_name}")
...
def api_update_device(device_name: str, device: DeviceInput):
    ...
    new_name = device.name if device.name else device_name
    if new_name != device_name and new_name in inventory:
        raise HTTPException(400, f"Tên '{new_name}' đã tồn tại!")
    ...
    save_inventory(inventory)
    add_log("INFO", "INVENTORY", f"Cập nhật thiết bị: {device_name}")
    return {"status": "success", "message": f"Đã cập nhật thiết bị {new_name}"}
...
def api_scan_network(subnet: str, username: str = "admin", password: str = ""):
    ...
    if not password:
        add_log("WARNING", "SCANNER",
                "Quét với password RỖNG - cac thiet bi tim thay se khong the backup/discovery "
                "cho den khi ban cap nhat lai password qua API PUT /api/inventory/{name}")
    ...
    try:
        ...
        add_log("ERROR", "SCANNER", f"Lỗi: {get_real_error(e)}")
        raise HTTPException(500, f"Lỗi khi quét: {get_real_error(e)}")
    finally:
